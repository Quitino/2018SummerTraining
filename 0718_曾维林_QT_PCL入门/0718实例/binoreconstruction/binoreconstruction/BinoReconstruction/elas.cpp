...
void Elas::operator()( InputArray _left, InputArray _right,
	                                 OutputArray _disp_left,OutputArray _disp_right)
{
	Mat left = _left.getMat(), right = _right.getMat();

	_disp_left.create(left.size(), CV_8UC1);
	_disp_right.create(left.size(), CV_8UC1);

	Mat disp_left = _disp_left.getMat();
	Mat disp_right = _disp_right.getMat();

	StereoElas(left,right,disp_left,disp_right);//Match

}
void Elas::StereoElas( const Mat& left, const Mat& right,Mat& disp_left,Mat& disp_right )
{
	int i_width  = left.cols;//
	int i_height = left.rows;//

	int o_width=0,o_hight=0;
	...
	int o_size = o_width*o_hight;

	// allocate memory for disparity images
	float* D1_data = (float*)malloc(o_size*sizeof(float));
	float* D2_data = (float*)malloc(o_size*sizeof(float));


	//int64 t1 = cv::getTickCount(), t2 = 0;

	const int32_t dims[3] = {i_width,i_height,i_width}; // bytes per line = width
	process(left.data,right.data, D1_data, D2_data,dims);//Match process
	
// 	t2 = getTickCount();
// 	cout<<"total process time: "<<(1000.0 * (t2 - t1) / cv::getTickFrequency())<<endl;

	// find maximum disparity for scaling output disparity images to [0..255]
	float disp_max = 0;
	for (int32_t i=0; i<o_size; i++) 
	{
	...
	// copy float to uchar
	uchar *_d_l = (uchar*)malloc(o_size*sizeof(uchar));
	uchar *_d_r = (uchar*)malloc(o_size*sizeof(uchar));

	for (int32_t i=0; i<o_size; i++)
	{
		_d_l[i] = (uchar)max(255.0*D1_data[i]/disp_max,0.0);
		_d_r[i] = (uchar)max(255.0*D2_data[i]/disp_max,0.0);
	}


    Mat d_l(cv::Size(o_width,o_hight), CV_8UC1,_d_l);
	Mat d_r(cv::Size(o_width,o_hight), CV_8UC1,_d_r);

	d_l.copyTo(disp_left);
	d_r.copyTo(disp_right);
...
void Elas::process (uint8_t* I1_,uint8_t* I2_,float* D1,float* D2,const int32_t* dims)
{
  ...
  // copy images to byte aligned memory
	...
	int64 t1 = cv::getTickCount();//starting

  //feature Descriptor
	Descriptor desc1(I1,width,height,bpl,param.subsampling);
	Descriptor desc2(I2,width,height,bpl,param.subsampling);//1

	int64 t2 = getTickCount();
	cout<<"feature time: "<<(1000.0 * (t2 - t1) / cv::getTickFrequency())<<endl;
	t2 = getTickCount();

  //Support Matches
	vector<support_pt> p_support = computeSupportMatches(desc1.I_desc,desc2.I_desc);//2
// 	ofstream fin("MatchPoint.txt");
// 	if (fin)
// 	{
// 		for (uint8_t i=0; i<D_can_width*D_can_height; i++)
// 		{
// 			fin << *(D_can+i) << endl;
// 		}
// 
...
//  fin.close();
  ...
  // if not enough support points for triangulation
	...
	cout<<"Support time: "<<(1000.0 * (t3 - t2) / cv::getTickFrequency())<<endl;
	t3 = getTickCount();
	  ...
	  cout<<"Triangulation time: "<<(1000.0 * (t4 - t3) / cv::getTickFrequency())<<endl;
	  t4 = getTickCount();
  ...
  //Disparity Planes
	  ...
	  cout<<"Matching time: "<<(1000.0 * (t5 - t4) / cv::getTickFrequency())<<endl;
	  t5 = getTickCount();
  ...
  //L/R Consistency Check
	  ...
	  cout<<"Consistency Check time: "<<(1000.0 * (t6 - t5) / cv::getTickFrequency())<<endl;
	  t6 = getTickCount();
  ...
  //Gap Interpolation
	  ...
	  cout<<"Postprocess time: "<<(1000.0 * (t7 - t5) / cv::getTickFrequency())<<endl;
