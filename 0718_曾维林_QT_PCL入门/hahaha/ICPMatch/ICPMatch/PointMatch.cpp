...
bool IPSG::CPointMatch::ICP_Process()
{
	...
	PointCloudT::Ptr filtered_cloud(new PointCloudT);   // 滤波目标点云
	PointCloudT::Ptr cloud_tr(new PointCloudT);         // 转换后的点云
	PointCloudT::Ptr cloud_icp(new PointCloudT);        // ICP输入点云
	...
	*cloud_tr = *cloud_icp;                     //备份cloud_icp赋值给cloud_tr为后期使用
	...
	icp.setMaximumIterations(m_nIterations);    //设置最大迭代次数iterations=true
	icp.setInputSource(cloud_icp);              //设置输入的点云
	icp.setInputTarget(filtered_cloud);         //目标点云
	...
	viewer.addPointCloud(filtered_cloud, cloud_in_color_h, "cloud_in_v1", v1);//设置原始的点云都是显示为白色
	...
	viewer.setSize(1280, 512);  // 可视化窗口的大小
